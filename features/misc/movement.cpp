...
void c_movement::minwalk(float speed) {
	if (speed <= 0.f)
		return;

	const auto min_speed = math::fastsqrt(globals::m_cmd->m_move.x * globals::m_cmd->m_move.x + globals::m_cmd->m_move.y * globals::m_cmd->m_move.y + globals::m_cmd->m_move.z * globals::m_cmd->m_move.z);
	if (min_speed <= 0.f)
		return;

	if (globals::m_cmd->m_buttons.has(IN_DUCK))
		speed *= 2.94117647f;
...
void c_movement::rotate(const qangle_t& wish_angles) {
	if (globals::m_cmd->m_view_angles.z != 0.f
		&& !globals::m_local->get_flags().has(FL_ONGROUND)) {
		globals::m_cmd->m_move.y = 0.f;
	}

	auto move_2d = vec2_t(globals::m_cmd->m_move.x, globals::m_cmd->m_move.y);

	if (const auto speed_2d = move_2d.length()) {
		const auto delta = globals::m_cmd->m_view_angles.y - wish_angles.y;
		...
		if (globals::m_local->get_move_type() == MOVE_TYPE_LADDER) {
			if (wish_angles.x < 45.f
				...
				&& globals::m_cmd->m_view_angles.x >= 45.f) {
				move_2d.x *= -1.f;
			}
		}
		else if (std::fabsf(globals::m_cmd->m_view_angles.x) > 90.f) {
			move_2d.x *= -1.f;
		}
	}
	...
	globals::m_cmd->m_move = vec3_t(
		math::clamp(move_2d.x, -max_forward_speed, max_forward_speed),
		math::clamp(move_2d.y, -max_side_speed, max_side_speed),
		math::clamp(globals::m_cmd->m_move.z, -max_up_speed, max_up_speed)
	);
}

void c_movement::compute_buttons() {
	globals::m_cmd->m_buttons.remove(IN_FORWARD | IN_BACK | IN_MOVERIGHT | IN_MOVELEFT);

	if (globals::m_local->get_move_type() == MOVE_TYPE_LADDER) {
		if (std::fabsf(globals::m_cmd->m_move.x) > 200.f) {
			globals::m_cmd->m_buttons.add(globals::m_cmd->m_move.x > 0.f ? IN_FORWARD : IN_BACK);
		}

		if (std::fabsf(globals::m_cmd->m_move.y) > 200.f) {
			globals::m_cmd->m_buttons.add(globals::m_cmd->m_move.y > 0.f ? IN_MOVERIGHT : IN_MOVELEFT);
		}

		return;
	}

	if (globals::m_cmd->m_move.x != 0.f) {
		globals::m_cmd->m_buttons.add(globals::m_cmd->m_move.x > 0.f ? IN_FORWARD : IN_BACK);
	}

	if (globals::m_cmd->m_move.y != 0.f) {
		globals::m_cmd->m_buttons.add(globals::m_cmd->m_move.y > 0.f ? IN_MOVERIGHT : IN_MOVELEFT);
	}
}